# preflight

A Claude Code plugin marketplace hosting a single plugin: **plan-before-code**
— a "plan before you code" workflow (epics and stories in a local plan
file, confirmed before implementation) that any Claude Code user can
install into their own projects.

See [`plan-before-code/README.md`](plan-before-code/README.md) for what the
plugin does and how to install it.

## Quick install

```
/plugin marketplace add rijosimon/preflight
/plugin install plan-before-code@preflight
/reload-plugins
```
